...
public class EJBManager implements EJBManagerRemote {
    ...
    Logger logger = LoggerFactory.getLogger(EJBManager.class);
    public EJBManager(){
        logger.debug("EJBManager");
    }
    public void connect(){
        try {
    ...
    //Create managers
    public void script_Manager() throws IOException {
        FileInputStream stream = new FileInputStream("Manager_Script.txt");
        InputStreamReader reader = new InputStreamReader(stream);
        BufferedReader br = new BufferedReader(reader);
        String linha2 = br.readLine();

        while(linha2 != null) {
            String [] manager = linha2.split(",");
            Manager m = new Manager(manager[0], manager[1], manager[2], Integer.parseInt(manager[3]),manager[4]);
            registerManager(m);
            linha2 = br.readLine();
        }
    ...
    public boolean deleteTrip(Trip trip){
        ...
        if(u.size()!=0){
            ...
            TypedQuery<Ticket> l = em.createQuery("SELECT DISTINCT t1 FROM Ticket t1 WHERE t1.trip.id=:trip_id", Ticket.class)
                    .setParameter("trip_id", trip.getId());
            em.getTransaction().begin();
            List<Ticket> tickets = l.getResultList();
